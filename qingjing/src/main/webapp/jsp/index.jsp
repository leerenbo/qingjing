...
<%@ page import="com.lrb.saas.core.APP" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<link rel="stylesheet prefetch" href="/cdn/js/antd/antd.css">

<link rel="stylesheet prefetch" href="/cdn/css/index.css">

<script src="/cdn/js/react-15.2.1/build/react.js"></script>
<script src="/cdn/js/react-15.2.1/build/react-dom.js"></script>
<script src="/cdn/js/babel-core-6.12.0/browser.min.js"></script>
<script src="/cdn/js/antd/antd.js"></script>
<script src="/cdn/js/FastJson-1.0.min.js"></script>
<script src="/cdn/js/jquery/jquery-3.1.0.js"></script>
<script src="/cdn/js/javascript-extends.js"></script>
...
function Saas(url){
	...
	this.columns=function(){
		...
		this.fields.forEach(
			field=>{
				if(field.type=='java.util.List<com.lrb.saas.core.model.qingjing.order.OrderItem>'){
					re.push({'title':field.name,'dataIndex':field.fieldName,render: text => {
																						const children=[];
																						for(var i=0;i<text.length;i++){
																							children.push(<div>{text[i].productionType.production.name+ text[i].productionType.name+ text[i].count}个</div>);
		...
		this.props.saas.fields.filter(field=>saasUtil.isFieldTypeSupport(field)).forEach(function(field,index){
			...
			case 'java.lang.String': 
				if(field.uiComponent=='Select'){
					let options=saasUtil.js(field.dictionaryUrl);
					let children = [];
					options.forEach(option=>{
					children.push(<Option key={option.value}>{option.text}</Option>);
					});

					input=	(<Select {...getFieldProps(field.fieldName+"$eq")}>
								{children}
							</Select>);
					break;
				}
		...
		this.props.saas.fields.filter(field=>saasUtil.isFieldTypeSupport(field)).forEach(function(field,index){
			...
			case 'java.lang.String': 
				if(field.uiComponent=='Select'){
					...
					input=	(<Select {...getFieldProps(field.fieldName)}>
								{children}
							</Select>);
					break;
				}
				if(field.uiComponent=='textarea'){
					input=<Input  type="textarea" rows={4} {...getFieldProps(field.fieldName)}/>;
					break;
				}
			...
			if(field.type.indexOf('model')>-1&&field.getUrl!=undefined&&field.getUrl!=''){
				let children = [];
		...
		this.props.saas.fields.filter(field=>saasUtil.isFieldTypeSupport(field)).forEach(function(field,index){
			...
			case 'java.lang.String': 
				if(field.uiComponent=='Select'){
					...
					console.info(data[field.fieldName]);
					input=	(<Select   {...getFieldProps(field.fieldName)}>
								{children}
							</Select>);
					break;
				...
				if(field.uiComponent=='textarea'){
					input=<Input  type="textarea" rows={4} {...getFieldProps(field.fieldName,{initialValue:data[field.fieldName]})}/>;
					break;
				}
	...
	render(){
		return (
			<div>
				...
				<Modal title="编辑" visible={this.state.putVisible} onCancel={this.cancelPutModal}
...
const SAASPage = React.createClass({
	getInitialState() {
		this.newTabIndex = 0;
		const menu =[{id:'a',title:'移动端',menuItems:[{id:'a1',title: '首页轮播图设置',saasInterfaceInfo:{url:'<%=APP.domain%>/indexScrollPicture'}}
																																								]},
					{id:'b',title:'内容管理',icon:'user',menuItems:[{id:'b1',title: '产品', saasInterfaceInfo:{url:'<%=APP.domain%>/production'}},
  		                                           {id:'b2',title: '产品类型',saasInterfaceInfo:{url:'<%=APP.domain%>/productionType'}},]},
					{id:'c',title:'订单管理',icon:'user',menuItems:[{id:'c1',title: '地址', saasInterfaceInfo:{url:'<%=APP.domain%>/address'}},
  		                                           {id:'c2',title: '优惠券',saasInterfaceInfo:{url:'<%=APP.domain%>/coupon'}},
  		                                           {id:'c3',title: '订单信息',saasInterfaceInfo:{url:'<%=APP.domain%>/orderList'}},
  		                                           {id:'c4',title: '支付信息',saasInterfaceInfo:{url:'<%=APP.domain%>/pay'}},]},
     		                                        ];
